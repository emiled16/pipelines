...
from ingestion.providers.who_alerts.provider import WhoAlertsProvider


def test_who_alerts_provider_yields_all_entries_without_checkpoint() -> None:
    async def run() -> None:
        provider = WhoAlertsProvider(
        ...
        assert [record.key for record in records] == ["alert-3", "alert-2", "alert-1"]
        assert records[0].metadata["page_url"] == (
...
def test_who_alerts_provider_stops_once_checkpoint_alert_is_reached() -> None:
    async def run() -> None:
        provider = WhoAlertsProvider(
            entries_loader=lambda checkpoint: _entries(
                [
                    {"id": "alert-3", "title": "Newest"},
                    {"id": "alert-2", "title": "Middle"},
                    {"id": "alert-1", "title": "Oldest"},
                ]
            ),
        )
        ...
        assert [record.key for record in records] == ["alert-3"]

    asyncio.run(run())


def test_who_alerts_provider_assigns_one_fetched_at_per_batch() -> None:
    async def run() -> None:
        provider = WhoAlertsProvider(
            entries_loader=lambda checkpoint: _entries(
                [
...
def test_who_alerts_provider_builds_checkpoint_with_http_cache_metadata() -> None:
    async def run() -> None:
        provider = WhoAlertsProvider(
            entries_loader=lambda checkpoint: _load_result(
                entries=[{"id": "alert-3", "title": "Newest"}],
                etag='"etag-2"',
        ...
        assert checkpoint == WhoAlertsCheckpoint(
            api_url=provider.api_url,
...
def test_who_alerts_provider_preserves_cursor_on_not_modified_response() -> None:
    async def run() -> None:
        provider = WhoAlertsProvider(
            entries_loader=lambda checkpoint: _load_result(
                entries=[],
        ...
        previous_checkpoint = WhoAlertsCheckpoint(
            api_url=provider.api_url,
            last_alert_id="alert-3",
            etag='"etag-2"',
            last_modified="Wed, 25 Mar 2026 10:15:00 GMT",
...
def test_who_alerts_provider_closes_entries_iterator_when_checkpoint_stops_iteration() -> None:
    async def run() -> None:
        from ingestion.providers.who_alerts.loader import WhoAlertsLoadResult
        ...
        provider = WhoAlertsProvider(
            entries_loader=lambda checkpoint: WhoAlertsLoadResult(entries=entries),
        )
        checkpoint = WhoAlertsCheckpoint(
            api_url=provider.api_url,
            last_alert_id="alert-2",
